...
load_watershed_data <- function(input_file, number_of_dimensions, pvalue_fraction, pvalue_threshold) {
  ...
  outliers_binary <- ifelse(abs(outlier_pvalues)<=pvalue_threshold,1,0)
	...
	data_input <- list(feat=as.matrix(feat), 
	                   outlier_pvalues=as.matrix(outlier_pvalues),
	                   outliers_binary=as.matrix(outliers_binary), 
	                   fraction_outliers_binary=as.matrix(fraction_outliers_binary),
...
initialize_model_params <- function(num_samples, 
                                    num_genomic_features, 
                                    number_of_dimensions, 
                                    phi_init, 
                                    theta_pair_init, 
                                    theta_singleton_init, 
                                    theta_init, 
                                    pseudoc, 
                                    lambda, 
                                    model_name, 
...
update_marginal_posterior_probabilities <- function(feat, discrete_outliers, model_params) {
	...
	if (tolower(model_params$model_name) == "river") {
		# Compute Expectation in CPP file ("independent_crf_exact_updates.cpp")
		posterior_list <- update_independent_marginal_probabilities_exact_inference_cpp(feat, 
		                                                                                discrete_outliers, 
		                                                                                model_params$theta_singleton, 
		                                                                                model_params$theta_pair, 
		                                                                                model_params$theta, 
		                                                                                model_params$phi$inlier_component, 
		                                                                                model_params$phi$outlier_component, 
		                                                                                model_params$number_of_dimensions, 
		                                                                                get_number_of_edge_pairs(model_params$number_of_dimensions), 
		                                                                                TRUE)
	} else if (tolower(model_params$model_name) == "watershed_exact") {
		# Compute Expectation in CPP file ("independent_crf_exact_updates.cpp")
		posterior_list <- update_marginal_probabilities_exact_inference_cpp(feat, 
		                                                                    discrete_outliers, 
		                                                                    model_params$theta_singleton, 
		                                                                    model_params$theta_pair, 
		                                                                    model_params$theta,
		                                                                    ...
		                                                                    model_params$phi$outlier_component, 
		                                                                    model_params$number_of_dimensions, 
		                                                                    choose(model_params$number_of_dimensions, 2), 
		                                                                    TRUE)
		...
		posterior_list <- update_marginal_probabilities_vi_cpp(feat, 
		                                                       ...
		                                                       model_params$number_of_dimensions, 
		                                                       choose(model_params$number_of_dimensions, 2), 
		                                                       model_params$vi_step_size, 
		                                                       model_params$vi_thresh, 
		                                                       model_params$posterior, TRUE)
	} else {
	  stop(sprintf("%s is not one of 'RIVER', 'Watershed_exact', 'Watershed_approximate'.", model_params$model_name))
	}
...
compute_exact_crf_likelihood_for_lbfgs <- function(x, 
                                                   feat, 
                                                   discrete_outliers, 
                                                   posterior, 
                                                   posterior_pairwise, 
	...
	if (tolower(model_name) == "watershed_exact") {
		...
		log_likelihood <- compute_crf_likelihood_exact_inference_cpp(posterior, 
		                                                             ...
		                                                             theta_pair, 
		                                                             theta, 
		                                                             phi$inlier_component, 
		                                                             phi$outlier_component, 
		                                                             number_of_dimensions, 
		                                                             lambda, 
		                                                             lambda_pair, 
		                                                             lambda_singleton)
	} else if (tolower(model_name) == "river") {
		# Following function comes from CPP code 'independent_crf_exact_updates.cpp'
		log_likelihood <- compute_independent_crf_likelihood_exact_inference_cpp(posterior, 
		                                                                         posterior_pairwise, 
		                                                                         feat, 
		                                                                         discrete_outliers, 
		                                                                         theta_singleton, 
...
compute_exact_crf_gradient_for_lbfgs <- function(x, 
                                                 feat, 
                                                 discrete_outliers, 
                                                 posterior, 
                                                 posterior_pairwise, 
	...
	if (tolower(model_name) == "watershed_exact") {
		# Following function comes from CPP code: 'crf_exact_updates.cpp'
		mu_list <- update_marginal_probabilities_exact_inference_cpp(feat, 
		                                                             discrete_outliers, 
		                                                             theta_singleton, 
		                                                             theta_pair, 
		                                                             theta, 
		...
		# Following function comes from CPP code 'independent_crf_exact_updates.cpp'
		mu_list <- update_independent_marginal_probabilities_exact_inference_cpp(feat, 
		                                                                         discrete_outliers, 
		                                                                         theta_singleton, 
		                                                                         theta_pair, 
		                                                                         theta, 
		                                                                         ...
		                                                                         phi$outlier_component, 
		                                                                         number_of_dimensions, 
		                                                                         get_number_of_edge_pairs(number_of_dimensions), 
		                                                                         FALSE)
		mu <- mu_list$probability
		mu_pairwise <- mu_list$probability_pairwise
	}
...
compute_exact_crf_pseudolikelihood_for_lbfgs <- function(x, 
                                                         feat, 
                                                         discrete_outliers, 
                                                         posterior, 
                                                         posterior_pairwise, 
	...
	log_likelihood <- compute_pseudolikelihood_crf_likelihood_exact_inference_cpp(posterior, 
	                                                                              posterior_pairwise, 
	                                                                              feat, 
	                                                                              discrete_outliers, 
	                                                                              theta_singleton, 
...
compute_exact_crf_pseudolikelihood_gradient_for_lbfgs <- function(x, 
                                                                  feat, 
                                                                  discrete_outliers, 
                                                                  posterior, 
                                                                  posterior_pairwise, 
	...
	mu_list <- update_pseudolikelihood_marginal_probabilities_exact_inference_cpp(feat, 
	                                                                              discrete_outliers,
	                                                                              posterior,
	                                                                              theta_singleton,
	                                                                              theta_pair,
	...
	grad_pair <- (2.0*colSums(posterior_pairwise) - 
	                colSums(mu_pairwise1) - 
	                colSums(mu_pairwise2))*(1/nrow(posterior_pairwise)) - 2.0*lambda_pair*theta_pair[1,]
...
map_crf <- function(feat, discrete_outliers, model_params) {
	...
	if (tolower(model_params$model_name) == "river" | tolower(model_params$model_name) == "watershed_exact") {
		lbfgs_output <- lbfgs(compute_exact_crf_likelihood_for_lbfgs, 
		                      ...
		                      posterior=model_params$posterior, 
		                      posterior_pairwise=model_params$posterior_pairwise, 
		                      phi=model_params$phi, lambda=model_params$lambda, 
		                      lambda_pair=model_params$lambda_pair, 
		                      lambda_singleton=model_params$lambda_singleton, 
	...
	if (lbfgs_output$convergence != 0 & lbfgs_output$convergence != 2) {
		warning(paste0("LBFGS optimazation on CRF did not converge. It reported convergence error of: ", lbfgs_output$convergence))
		warning(lbfgs_output$message)
	}
	...
	for (dimension in 1:model_params$number_of_dimensions) {
 	...
 	model_params$theta_pair <- matrix(lbfgs_output$par[(model_params$number_of_dimensions + 
 	                                                      (model_params$number_of_dimensions*ncol(feat)) + 1):length(lbfgs_output$par)], 
 	                                  ncol=get_number_of_edge_pairs(model_params$number_of_dimensions),byrow=TRUE)
...
check_convergence <- function(model_params, phi_old, theta_old, theta_singleton_old, theta_pair_old, iter, max_iter) {
	...
	if (tolower(model_params$model_name) == "river") {
		total_norm = (norm(model_params$theta - theta_old) + 
		                ...
		                norm(model_params$phi$outlier_component - phi_old$outlier_component))/total_params_river
	} else {
		total_norm = (norm(model_params$theta - theta_old) + 
		                norm(as.matrix(model_params$theta_singleton) - as.matrix(theta_singleton_old)) + 
		                norm(model_params$theta_pair - theta_pair_old) + 
		                norm(model_params$phi$inlier_component - phi_old$inlier_component) + 
		                norm(model_params$phi$outlier_component - phi_old$outlier_component))/total_params_watershed
	}

	message(paste0("Average total norm: ", total_norm))

	# Check for convergence
	if (total_norm < 1e-4) {
		converged = TRUE
		message("Watershed converged")
	} else if (iter == max_iter) {
		converged = TRUE
		message("Watershed converged due to reaching max iteration")
	}
...
### Fit Watershed Model
train_watershed_model <- function(feat, 
                                  discrete_outliers, 
                                  phi_init, 
                                  theta_pair_init, 
                                  theta_singleton_init, 
  ...
  model_name <- tolower(model_name)
	# Put model parameters in an easy to handle data structure
	model_params <- initialize_model_params(dim(feat)[1], 
	                                        dim(feat)[2], 
	                                        number_of_dimensions, 
	                                        phi_init, 
	                                        theta_pair_init, 
